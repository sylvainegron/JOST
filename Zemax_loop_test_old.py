# -*- coding: utf-8 -*-
"""
Created on Mon Jul 18 17:12:14 2016
...
measure_vect=measure_vect[0:8]    #keep the first zernikes only
residual_vect=residual_vect[0:8]

actuator_value=np.array([1.,1.,1.,1.])

# Getting the noise covarience matrix 
noise_level=2.
Cn = np.diag(np.ones(nb_FoV*7)*noise_level )  # size: 2D nb_WFS * Zmax , nb_WFS * Zmax


nbr_iterations=10
wf_simulation = np.zeros((8*nb_FoV,nbr_iterations+1)) 
...
for iteration in range(nbr_iterations):
    ...
    for ind in range(nb_FoV):
        ...
        link.zSetSurfaceParameter(9, 4, -configFile_python[6+8*ind]/1000./2.)
        link.zSetSurfaceParameter(9, 3, -configFile_python[6+8*ind+1]/1000./2.)
        ...
        #L1    
        # get the push
        link.zSetSurfaceData(18, 3, np.sum(misalignment[0,:])+actuator_value[0])
        ...
        #L2
        # get the push
        link.zSetSurfaceData(24, 3, np.sum(misalignment[1,:])+actuator_value[1])
        coefZern_plus = link.zGetZernikeCoef(Zmax=153)    
        # get the pull
        link.zSetSurfaceData(24, 3, np.sum(misalignment[1,:])-actuator_value[1])
        coefZern_minus = link.zGetZernikeCoef(Zmax=153)    
        ...
        #L3   
        # get the push
        link.zSetSurfaceData(31, 3, np.sum(misalignment[2,:])+actuator_value[2])
        coefZern_plus = link.zGetZernikeCoef(Zmax=153)
        # get the pull
        link.zSetSurfaceData(31, 3, np.sum(misalignment[2,:])-actuator_value[2])
        coefZern_minus = link.zGetZernikeCoef(Zmax=153)
        ...
        #Camera    
        # get the push
        link.zSetSurfaceData(39, 3, np.sum(misalignment[3,:])+actuator_value[3])
        coefZern_plus = link.zGetZernikeCoef(Zmax=153)
        # get the pull
        link.zSetSurfaceData(39, 3, np.sum(misalignment[3,:])-actuator_value[3])
        coefZern_minus = link.zGetZernikeCoef(Zmax=153)
        ...
        Misalignment_SVD[:,iteration]=np.dot(MI,wf_simulation[:,iteration]-measure_vect)
