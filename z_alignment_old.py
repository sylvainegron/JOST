...
#    DM.stdin.flush()
    ...
    nbr_iterations=5
    Misalignment=np.zeros((nbr_iterations,4))
    Misalignment_motor_motion=np.zeros((nbr_iterations,4))
    wf_simulation=np.zeros((nbr_iterations,(zmax-5)*nb_FoV))
    
    for ind in range(nbr_iterations):
        Misalignment[ind],Misalignment_motor_motion[ind]=Zalign.MMSE_reconstruction(Date,configFile_python,noise_level,pert,IM,measure_vect,residual_vect)
        wf_simulation[ind]=Simulated_Wavefront_Zalignment(Date,configFile_python,Misalignment)
    ...
    print "move the Camera by ",Misalignment[3],"mm"    
